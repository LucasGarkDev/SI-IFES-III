...
public class GerInterGrafica {
    ...
    public void carregarComboCidades(JComboBox combo) {
        try {
            List lista = gerDominio.listarCidades();                
    ...
    public void carregarComboLanches(JComboBox combo) {
        try {
            List lista = gerDominio.listarLanches();                
            combo.setModel(  new DefaultComboBoxModel( lista.toArray() )   );
        } catch (ClassNotFoundException | SQLException ex) {
            JOptionPane.showMessageDialog(janCadPedido, ex, "Cadastro de Pedido", JOptionPane.ERROR_MESSAGE );
        }
    }
